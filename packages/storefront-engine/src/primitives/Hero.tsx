  ...
  return (
    <section className="relative overflow-hidden rounded-2xl border border-brand/20 bg-brand/5 shadow-elevation-1">
      <div className="pointer-events-none absolute inset-y-0 right-0 hidden w-[42%] overflow-hidden lg:block" aria-hidden="true">
        ...
        {!product?.image && <div className="absolute right-16 top-1/2 h-28 w-44 -translate-y-1/2 rotate-6 rounded-2xl border border-brand/20 bg-surface/80 shadow-elevation-2" />}
        {!product?.image && <div className="absolute right-44 top-[58%] h-20 w-32 -rotate-6 rounded-xl bg-brand/15" />}
      </div>

      <div className={`relative z-10 grid ${product?.image ? 'lg:grid-cols-[minmax(0,1.05fr)_minmax(320px,0.95fr)]' : ''}`}>
        ...
        {product?.image && productHref && (
          <Link href={productHref} className="group relative min-h-80 overflow-hidden border-t border-brand/10 bg-surface lg:min-h-full lg:border-l lg:border-t-0" aria-label={`View ${product.name}`}>
