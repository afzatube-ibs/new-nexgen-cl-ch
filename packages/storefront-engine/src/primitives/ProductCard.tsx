...
/**
 * `STOREFRONT_COMPONENT_ENGINE.md` §2's `ProductCard` primitive — v2,
 * rebuilt for Beta Milestone 2.5's own "Professional Product Card v2"
 * spec on top of Beta Milestone 2's own real rebuild. Still a real Server
 * Component for everything that doesn't need client state (image, link,
 * price, stock) — `ProductQuickActions` (a real Client Component) is the
 * one interactive subtree, split out for the same Server/Client boundary
 * reason `StoreHeader.tsx`'s own docblock documents.
 *
 ...
 * - Badges: only `status`-derived (`StockBadge`) is real. New/Hot/
 *   Trending/Flash Sale/Limited Stock/Imported/Warranty/COD have no real
 *   backing signal — not rendered (see audit for the real fields each
 *   would need).
 ...
 * - Skeleton loading: `ProductCardSkeleton` below, unchanged.
 */
export function ProductCard({ product, href, brandName, price, compareAtPrice }: ProductCardProps) {
  return (
    ...
    >
      <Link
        href={href}
        className="focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus focus-visible:ring-offset-2 rounded-t-lg"
...
export function ProductCardSkeleton() {
  return (
    <div className="flex flex-col overflow-hidden rounded-lg border border-border bg-surface" aria-hidden="true">
