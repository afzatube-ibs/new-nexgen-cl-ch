...
**Covers:** #26 Payments
**Phase:** 1 (P0)
**New module. Delivered Bangladesh-first (2026-08-05), per the Product Owner's explicit direction, at greater depth than this entry's original scope: a full provider-agnostic gateway architecture (Gateway Registry/Resolver/Factory, one shared `PaymentGatewayContract` every gateway implements), with Cash On Delivery and Bank Transfer as fully functional, first-class, no-external-network gateways, and SSLCommerz/bKash/Nagad as production-ready architecture against each provider's own real, documented API — never a placeholder, gated by that gateway's own credential-driven `isAvailable()` check when this installation has not configured them.**
- **Purpose:** The integration boundary to external payment processors, per `ARCH:SYSTEM_CONTEXT` — Payments processes payment ONLY for an Order that already exists; it never decides what an Order costs.
- **Responsibilities:** Payment initiation, authorization, capture, cancellation, and void against an existing Order; the Payment status lifecycle (pending → authorized → captured, with failed/cancelled/voided early exits); idempotent initiation and duplicate-payment protection (at most one active Payment per Order at a time); webhook processing with mandatory signature verification and replay protection; reconciliation against each gateway's own query API. Never calculates prices, taxes, or discounts, never creates an Order, never manages inventory or customers — those remain Pricing's, Promotions's, Orders's, Inventory's, and Customers's own responsibilities respectively; never stores card data itself, per `08_SECURITY_STANDARD`.
- **Dependencies:** Orders only, as a real code-level dependency — read-only, exercised exactly once per payment (at initiation, to snapshot `grand_total`/`currency_code`/`customer_id`), mirroring Orders' own narrow dependency on Customers. Not Checkout: Checkout has already produced a durable Order by the time Payments is ever involved, so Payments has no code-level reason to touch it.
- **Public Contracts:** Initiate payment, capture (manual confirmation for Cash On Delivery/Bank Transfer), cancel, void, list/show, payment-methods listing, bank-transfer proof/approve/reject, one webhook endpoint per gateway (SSLCommerz, bKash, Nagad).
- **Events:** `PaymentInitiated`, `PaymentAuthorized`, `PaymentCaptured`, `PaymentFailed`, `PaymentCancelled`, `PaymentVoided`. (`PaymentRefunded`, named in this entry's original draft, is not published in this delivery — refunds are Returns' future responsibility, per Orders' own precedent of leaving refunds out of scope; `Gateways\Contracts\RefundableGateway` is the extension point a future Returns module extends, implemented today only by `Gateways\BkashGateway`, per the master plan's own "Refund Extension Point" instruction for bKash specifically.)
- **Data Ownership:** Payment records, their attempt/transaction ledger, and inbound webhook delivery logs — Sensitive, per `DATA:CLASSIFICATION`; every mutation and every webhook-verification outcome (including rejected/unmatched deliveries) is audited, not mutations alone, per `SECURITY:AUDIT_LOGGING`'s Sensitive-data floor. Never raw card data — Bangladesh's first-class Phase 1 methods (Cash On Delivery, Bank Transfer, and every redirect/token-based gateway) never collect it in the first place.
- **Security Considerations:** The single highest-severity security boundary in the platform besides Identity & Access — full `SECURITY:SECURITY_BOUNDARIES` external-integration treatment. "Never trust gateway callbacks. Always verify signatures": every webhook is verified before any Payment is touched, and an inbound delivery this module cannot verify or cannot match to a Payment is recorded (`rejected`/`unmatched`) and audited, never silently dropped or guessed at. Duplicate Payment Protection and idempotent initiation are enforced by locking on the Order, not merely checked optimistically. Webhook replay protection is enforced by a database-level unique index (`payment_webhook_events` on `(gateway_code, event_reference)`), not application logic alone.
- **Future Extension Points:** Additional payment processors (Stripe, PayPal, Paddle, Lemon Squeezy, Wise, Adyen, Square, Authorize.Net) added as new `PaymentGatewayContract` implementations plus one `Gateways\GatewayFactory` case and one `config/payments.php` entry — no change to Models\Payment, any Actions\* class, or any controller. Cash On Delivery's own extension points (fee, by shipping zone, by store, by product, by customer group, risk rules) are named in `config/payments.php` but not implemented in Phase 1. Refunds, per `Gateways\Contracts\RefundableGateway`.
- **Complexity:** Very High.
- **Acceptance Criteria:** No card data ever appears in logs, per `ENGINEERING:LOGGING_PRINCIPLES`; every payment event and every webhook-verification outcome is audited; no single request atomically spans more than one aggregate's mutation, per `DATA:TRANSACTION_BOUNDARIES` — Payments' own Actions each transact against the Payment aggregate alone, never opening a transaction that also touches Orders'.
