...
- **Estimated files**: ~25–30 (Backend: new auth Actions/Controllers/Requests in Customers, ~8–10; Gateway: a new Category-C-style customer-session composition layer, ~6–8; Storefront: login/register/account pages + auth state, ~10–12; tests throughout).
- **Estimated complexity**: High — this is a new, security-sensitive auth surface, not a composition task; needs its own careful session/token design decision (a real design decision, not a build-time one — flag for Product Owner sign-off before implementation).
- **Dependencies**: None hard (Customers/Orders backends already exist and are ready to be read from). Everything downstream of "a shopper has an identity" (order history, address reuse, Reviews-with-verified-purchase, wishlists) becomes real once this ships.
- **Production value**: Very high, foundational — but the largest, riskiest single milestone on this list.
- **Expected completion after**: Customers 55% → ~85%.

### Milestone 6 — Admin: Identity & Access (User & Role Management UI)
- **Objective**: A real Admin UI for inviting staff, assigning roles, and viewing the permission matrix — the backend RBAC is mature and completely unreachable by any UI today.
- **Estimated files**: ~12–15 (user list/invite/edit, role list/edit, permission-assignment UI, ~10–12 Storefront/Admin files; minor backend additions only if an "invite" flow doesn't already exist as an Action, ~0–3 files).
- **Estimated complexity**: Medium — mostly UI over an already-complete backend; complexity comes from correctly gating the UI itself (a role editor must not let a non-Administrator escalate their own privileges).
- **Dependencies**: None.
- **Production value**: High — currently the only way to grant a second person staff access is direct database/API manipulation, which is not viable for a real merchant.
- **Expected completion after**: Identity & Access 65% → ~90%.

### Milestone 7 — Admin: Fulfillment & Returns UI
- **Objective**: Real Admin surfaces for both currently UI-less back-office modules: shipment list/pick/pack/dispatch for Fulfillment, and RMA list/approve/refund for Returns.
- **Estimated files**: ~20–25 (roughly split evenly between the two modules; both backends are already complete, so this is UI-only plus any missing customer-facing return-request endpoint).
- **Estimated complexity**: Medium — two related but independent Admin modules; could be split into two half-sized milestones if preferred.
- **Dependencies**: None hard. Soft benefit: Returns' refund step already correctly reacts to Payments via the event bus, so no new cross-domain work is needed.
- **Production value**: High — an operator cannot process a real shipment or a real return through any UI today.
- **Expected completion after**: Fulfillment 60% → ~85%; Returns 55% → ~85%.

### Milestone 8 — Dashboard Real Widgets
- **Objective**: Implement the named widgets (Today's Orders, Today's Revenue, This Month Revenue, Pending Orders, Low Stock, Recent Orders, Latest Customers, Top Selling Products, Recent Activity) using the already-real `dashboardWidgets` registry — currently zero widgets are registered by any module.
- **Estimated files**: ~10–14 (one widget component + registration per metric, spread across Orders/Payments/Inventory/Customers/Catalog's own `module.ts` files, reusing the existing framework — no new framework code needed).
- **Estimated complexity**: Low-Medium — the hard part (the registry framework) is already built and tested; this is real data-fetching plus presentation per widget.
- **Dependencies**: Revenue widgets are meaningfully more useful after Milestone 2 (real prices), but the framework does not require it.
- **Production value**: High — the first thing an operator sees after logging in is currently an empty shell.
- **Expected completion after**: Dashboard 15% → ~80%.
...
- **Estimated files**: ~10–14 (Store Configuration panel, Localization panel, plus wiring each module's `module.ts` to actually call `registerSettingsPanel`).
- **Estimated complexity**: Low — the framework exists and is tested; this is populating it.
- **Dependencies**: None.
- **Production value**: Medium — currently invisible to a merchant unless they go looking for the empty Settings page, but store name/currency/timezone are basic expectations for any real deployment.
- **Expected completion after**: Store Configuration 45% → ~75%; Localization 55% → ~75%.
