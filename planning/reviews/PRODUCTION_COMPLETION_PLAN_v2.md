...
### Milestone 11 — Reviews Foundation (new module) — ✅ Shipped
...
- **Verification against the repository (before implementation)**: confirmed fully accurate on every count — the one milestone this session found the plan's own characterization correct without any correction needed. No `Review` model/controller/migration existed anywhere; the Storefront components were real, complete, and genuinely unwired.
- **What actually shipped**: a complete Reviews domain (`app/Domains/Commerce/Reviews`) — real bidirectional moderation state machine, verified-purchase check against Orders (Reviews' one real, documented cross-domain dependency, mirroring Payments'), audit log, permissions granted to the Gateway's own service credential; Gateway composition (`routes/reviews.ts`, Category A cached reads + Category C authenticated write); `storefront-engine` client (`gateway/reviews.ts`, `ReviewForm`); real Storefront wiring (`app/products/[idSlug]/page.tsx`, `app/api/reviews/route.ts`) — `RatingSummary`/`ReviewList` now render real data, with a real, working submission form.
- **Real bugs found and fixed**: an undefined-array-key crash on a title-less review submission; a platform-wide Gateway gap where every module's real HTTP 409 fell through to a fabricated 502 (discovered via Reviews' own duplicate-review conflict, fixed for every module); a pre-existing Milestone 8 Arch-test regression (`OrderMetricsController`'s DB usage) that had gone unrun since it shipped.
...
### Milestone 13 — Reviews Admin Moderation UI — ✅ Shipped
- **Objective**: Close the one real, disclosed gap Milestone 11 (Reviews Foundation) named in its own completion report — every moderation capability (approve/reject/respond/delete) was real and reachable via the permission-gated API, but no Admin screen existed to reach it.
- **Not in this plan's original Part 3 scope** — identified during Milestone 11's own verification pass and added here as a real, completed follow-up, per the standing instruction to keep this roadmap synced to the actual repository rather than treat the original document as exhaustive.
- **What actually shipped**: `packages/api-client/src/reviews/` (new typed REST layer, mirroring `returns/`'s own shape); a new Admin `reviews` module — a real, filterable/paginated Reviews list with per-row Approve/Reject/Respond/Delete actions (each offered per the review's own real, bidirectional `Review::TRANSITIONS`), and a Reviews Activity audit-log screen.
...
### Milestone 16 — Notifications Admin UI — ✅ Shipped
- **Objective**: Close the same class of gap Milestone 13 closed for Reviews — Notifications' backend (`app/Domains/Operations/Notifications`) has been real and complete since Milestone 3, but no Admin screen ever existed to see what was sent, retry a failure, cancel a pending send, or manage templates.
- **Verification against the repository (before implementation)**: every capability this milestone exposes already existed and required no backend changes — real routes for templates (index/store/show/update, no delete), notifications (index/show/retry/cancel), and an audit log; confirmed `retry()` has no optimistic-lock check (only `cancel()` does) and `NotificationResource` never exposes rendered message content, both by reading the controllers/resources directly rather than assuming.
- **What actually shipped**: `packages/api-client/src/notifications/` templates/workflow/audit-log layers; a new Admin `notifications` module — Templates list (server-side search + channel filter, create/edit dialog with immutable code/channel/locale after creation, Activate/Deactivate), Notifications log (status/channel filters, status-gated Retry/Cancel), Notification Detail (Overview + real Delivery Timeline), and a Notifications Activity audit log.
- **A real, platform-wide UX bug found and fixed during this milestone's own live verification**: confirming a destructive `ConfirmDialog` action (e.g. "Cancel notification") from inside a `DataTable` row force-navigated to that row's own Detail page immediately after — `DialogContent` renders via a React portal, and React re-dispatches synthetic events through the *component* tree rather than the DOM tree, so the confirm click still bubbled past the portal boundary to the row's `onRowClick`. Since `ConfirmDialog` is the platform's one shared destructive-confirmation component, this reproduced on every list page combining a row-action `ConfirmDialog` with `onRowClick` — fixed at the root in three shared framework files (`ConfirmDialog.tsx`, and `packages/ui`'s `Dialog.tsx`/`DropdownMenu.tsx` for the identical, not-yet-reproduced variant via their own portal-rendered controls), with a new regression test reproducing the exact row+dialog composition.
- **Completion**: Notifications now has no further known gaps. See `MILESTONE_16_NOTIFICATIONS_ADMIN_UI_COMPLETION_REPORT.md`.
