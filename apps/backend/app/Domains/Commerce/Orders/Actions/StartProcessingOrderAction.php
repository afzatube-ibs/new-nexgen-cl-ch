...
/**
 * "Order status lifecycle": confirmed -> processing — see Models\Order's
 * docblock for the full state graph.
 */
final readonly class StartProcessingOrderAction
{
    ...
    public function execute(Order $order, int $expectedVersion, ?string $actorId): Order
    {
        return DB::transaction(function () use ($order, $expectedVersion, $actorId) {
            ...
            if (! $order->canTransitionTo(Order::STATUS_PROCESSING)) {
                throw new InvalidOrderStatusTransitionException($order->id, $order->status, Order::STATUS_PROCESSING);
            }
            ...
            $order->timelineEvents()->create([
                ...
                'description' => "Order moved to processing (was {$previousStatus}).",
                'occurred_at' => now(),
            ]);
