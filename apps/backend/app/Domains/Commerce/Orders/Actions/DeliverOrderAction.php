...
/**
 * "Order status lifecycle": shipped -> delivered (terminal) — see
 * Models\Order's docblock for the full state graph.
 */
final readonly class DeliverOrderAction
{
    ...
    public function execute(Order $order, int $expectedVersion, ?string $actorId): Order
    {
        return DB::transaction(function () use ($order, $expectedVersion, $actorId) {
            ...
            if (! $order->canTransitionTo(Order::STATUS_DELIVERED)) {
                throw new InvalidOrderStatusTransitionException($order->id, $order->status, Order::STATUS_DELIVERED);
            }
            ...
            $order->timelineEvents()->create([
                ...
                'description' => "Order delivered (was {$previousStatus}).",
                'occurred_at' => now(),
            ]);
