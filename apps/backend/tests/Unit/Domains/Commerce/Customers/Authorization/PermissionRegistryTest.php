...
it('declares every permission under the customers module', function () {
    foreach (PermissionRegistry::definitions() as $definition) {
        expect($definition->module)->toBe('customers');
        expect($definition->key)->toStartWith('customers.');
    }
});
...
it('declares at least one permission for every resource this module exposes', function () {
    ...
    foreach (['customers.customers.view', 'customers.customers.manage', 'customers.audit_log.view'] as $expected) {
