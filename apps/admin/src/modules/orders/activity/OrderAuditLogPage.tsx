...
import { ORDER_TARGET_TYPE, ORDER_NOTE_TARGET_TYPE, type OrderAuditLogDTO } from '@nexgen/api-client';
...
import { useOrderAuditLogs, useStaffDirectory } from '../shared/queries.js';

type TargetTypeFilter = 'all' | typeof ORDER_TARGET_TYPE | typeof ORDER_NOTE_TARGET_TYPE;
const PAGE_SIZE = 25;

/**
 * Orders Audit Log — `orders.audit_log.view`, a real, complete, server-
 * paginated browse of `GET /orders/audit-logs` (`AuditLogController::index`,
 * Orders' own). A dedicated permission from `orders.orders.*` — the same
 * shape Customers' own module already established, and this screen mirrors
 * that one's design directly (Type + Staff filters, no `target_id` filter,
 * server pagination), since the real backend constraint is identical.
 */
export function OrderAuditLogPage() {
  ...
  const { data, status, refetch } = useOrderAuditLogs({
  ...
  const columns: DataTableColumn<OrderAuditLogDTO>[] = [
    {
      id: 'when',
      header: 'When',
      cell: (row) => (
  ...
  return (
    <div>
      <CrudPageLayout
        header={{ title: 'Orders Audit Log', description: 'Every recorded change to an order — status transitions and notes.' }}
        toolbar={
          <Toolbar
            filters={
              <FilterBar
              ...
              >
                <div className="flex flex-col gap-3">
                  <Select
                    ...
                    options={[
                      { value: 'all', label: 'All' },
                      { value: ORDER_TARGET_TYPE, label: 'Order' },
                      { value: ORDER_NOTE_TARGET_TYPE, label: 'Note' },
                    ]}
                  />
