...
import { REVIEW_TARGET_TYPE, type ReviewsAuditLogDTO } from '@nexgen/api-client';
...
import { useReviewsAuditLogs, useStaffDirectory } from './queries.js';

type TargetTypeFilter = 'all' | typeof REVIEW_TARGET_TYPE;
const PAGE_SIZE = 25;

/**
 * Reviews Audit Log — `reviews.audit_log.view`, a real, complete,
 * server-paginated browse of `GET /reviews/audit-logs`
 * (`AuditLogController::index`, Reviews' own). Every review lifecycle
 * change (submitted/approved/rejected/responded/deleted) is recorded here
 * — mirrors Returns'/Shipping's/Fulfillment's own identically-shaped audit
 * log page design directly (Type + Staff filters, no `target_id` filter,
 * server pagination), since the real backend constraint is identical.
 */
export function ReviewsAuditLogPage() {
  const [targetType, setTargetType] = useState<TargetTypeFilter>('all');
  const [actorId, setActorId] = useState<string>('all');
  const [page, setPage] = useState(1);

  const { data, status, refetch } = useReviewsAuditLogs({
  ...
  const columns: DataTableColumn<ReviewsAuditLogDTO>[] = [
    {
      id: 'when',
      header: 'When',
      cell: (row) => (
  ...
  return (
    <div>
      <CrudPageLayout
        header={{ title: 'Reviews Activity', description: 'Every recorded change to a customer review.' }}
        toolbar={
          <Toolbar
            filters={
              <FilterBar
              ...
              >
                <div className="flex flex-col gap-3">
                  <Select
                    ...
                    options={[
                      { value: 'all', label: 'All' },
                      { value: REVIEW_TARGET_TYPE, label: 'Review' },
                    ]}
                  />
