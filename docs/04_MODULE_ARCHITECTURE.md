...
| 1.3 | 2026-08-05 | Formally added `MODULE:CHECKOUT` and `MODULE:PAYMENTS` (§5, Commerce domain), both already implemented and delivered in prior sessions but not yet reflected in this document's own Change Log or `MODULE:STABILITY` (§8) classification — a gap this entry corrects rather than a new decision. Classified both Evolvable under §8 (Checkout's entry was already present in the Evolvable list; Payments' was missing entirely and is added here). No existing module boundary, dependency, or coupling rule was changed | Found during the Phase 1 Backend Completion Review (2026-08-05): `MODULE:AUTHORITY` requires every implemented module to carry a stability classification and a Change Log entry; Checkout and Payments had neither fully in place despite being implemented |
| 1.4 | 2026-08-08 | Formally added `MODULE:NOTIFICATIONS` (§6, Operations domain), per `MODULE:AUTHORITY`'s requirement that a module boundary exist here before implementation begins — `planning/IMPLEMENTATION_MASTER_PLAN.md` names the module (#33) but this document had never defined its boundary, only a passing mention at §4's Localization & Currency entry ("the translation surface Catalog, CMS, and Notifications content read from"). Classified Evolvable under §8, alongside its Operations siblings. Placed in Operations rather than Platform or Growth: `ARCH:DOMAIN_MAP`'s own Platform description and this document's §10 ("Out of Platform: no Platform module may depend on any module outside Platform, directly or via events") structurally rule out Platform, since Notifications must react broadly to Commerce and Operations events; Growth is ruled out because both `ARCH:DOMAIN_MAP` and this document's own §7 state Growth is "designed for, not built in, Phase 1," and Notifications is being built now. Operations' own domain description — "Reacts to events from Commerce... without owning Commerce's data" — is an exact structural match, and is already an exercised, accepted pattern (Fulfillment already reacts to Orders' `OrderPlaced`). No existing module boundary, dependency, or coupling rule was changed | Product Owner directed Phase 1 implementation to continue in the master plan's stated order, which reaches this module next; `MODULE:AUTHORITY` requires this document to define it before that implementation begins |
| 1.5 | 2026-08-08 | Formally added `MODULE:SEARCH` (§5, Commerce domain), per `MODULE:AUTHORITY`'s requirement — `planning/IMPLEMENTATION_MASTER_PLAN.md` names it (#29, "Cross-Catalog (and later cross-CMS) search... Dependencies: Catalog (primary)") but this document had never defined its boundary at all, not even a passing mention. Placed in Commerce, matching the master plan's own stated dependency: Catalog is Commerce, and "within Commerce, direct module-to-module calls are permitted along the dependencies stated above" makes Search's own event subscriptions to Catalog's `ProductCreated`/`ProductUpdated`/`ProductArchived` a same-domain, direct relationship needing no cross-domain event-routing seam — unlike every module added in v1.2–v1.4, which each needed one. Classified Evolvable under §8, alongside its Commerce siblings. Scope confirmed with the Product Owner before implementation: this module owns only a derived, rebuildable Catalog product index (per `DATA:SEARCH_INDEXING` and the master plan's own "the search index can be fully rebuilt from Catalog... data alone with zero information loss" acceptance criterion) — Customer/Order/Store/Notification-template/audit-log search, requested in the same implementation instruction, is out of this module's accepted boundary (a materially larger, genuinely cross-domain federation the master plan does not scope here) and is instead satisfied by each owning module's own existing (or, where missing, newly added) list-endpoint filtering, within that module. No existing module boundary, dependency, or coupling rule was changed | Product Owner directed Phase 1 implementation to continue in the master plan's stated order, which reaches this module next — the last of the 19 Phase 1 modules; `MODULE:AUTHORITY` requires this document to define it before that implementation begins |
| 1.6 | 2026-09-07 | Formally added `MODULE:CMS` (§4, Platform domain), following the accepted CMS Foundation content model and the Product Owner's explicit production-completion authorization. CMS owns merchant-authored storefront Pages, Menus/Navigation, reusable Snippets, revisions, and their draft/publish lifecycle; it references Media and Localization through Platform public contracts and Catalog entities by identifier only. Classified Experimental under §8 for its first implementation. | neXgen V1 requires merchant-managed homepage/content/policy/navigation surfaces; `MODULE:AUTHORITY` requires this boundary to be accepted before persistence/API implementation begins. |
...
- **Payments** (`MODULE:PAYMENTS`) — the provider-agnostic integration boundary to external payment processors, per `ARCH:SYSTEM_CONTEXT`. Owns the Payment aggregate, its attempt/transaction ledger, webhook replay protection, and every gateway integration; never stores raw card data. Depends on Orders only, and only as a narrow, real, read-only code-level dependency (see Actions\InitiatePaymentAction's docblock) — it reads an Order's `grand_total`/`currency_code`/`customer_id` exactly once, at payment-initiation time, to snapshot the amount owed, mirroring the same pattern Orders itself already uses for its own narrow dependency on Customers. Payments never calculates a price, a tax, or a discount, never creates an Order, and never manages inventory or customers — those decisions have already been made by the time an Order exists; Payments only processes payment against one. Delivered Bangladesh-first (2026-08-05): Cash On Delivery and Bank Transfer as fully local, first-class gateways with no external network dependency; SSLCommerz, bKash, and Nagad as production-ready architecture against each provider's real, documented API shape, gated by `Gateways\Contracts\PaymentGatewayContract::isAvailable()` on that gateway's configured credentials. "Adding a new gateway should require ONLY: Implement PaymentGateway contract, Register the gateway" — the master plan's own acceptance criterion for this module's extensibility, made concrete by `Gateways\GatewayFactory`/`Gateways\GatewayRegistry`/`Gateways\GatewayResolver`.
- **Search** (`MODULE:SEARCH`) — cross-Catalog (and, once built, cross-CMS) product search: index maintenance, query handling, relevance ranking. Owns only a derived, read-optimized index over Catalog's own product data, per `DATA:SEARCH_INDEXING` — never a second source of truth, always rebuildable from Catalog alone. Depends on Catalog directly (same-domain), reacting to `ProductCreated`/`ProductUpdated`/`ProductArchived` to keep its index current. Results are always filtered to what a caller may lawfully see (a product's own `active` status and `search`/`catalog_search` visibility), never a caller-supplied bypass, per this module's own Security Considerations in the master plan.
...
- **Notifications** (`MODULE:NOTIFICATIONS`) — the platform's single outbound-communication surface (per `ARCH:SYSTEM_CONTEXT`'s "email/notification providers" external boundary): owns notification templates, the record of every notification attempted, and delivery status. Reacts broadly to events from Commerce and Operations (an order placed, a payment captured or refunded, a shipment dispatched or delivered, a return requested or refunded) rather than depending on any of those modules directly, exactly matching this domain's own "reacts to events... without owning [the triggering module's] data" pattern. Never depends on Growth (Growth may depend on Notifications, per `MODULE:COUPLING_RULES`, never the reverse).
